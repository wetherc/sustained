...
- A failed attempt is on record. A failed `up` or `down` writes that row only when nothing rolled it back: in a migration with `transactional=False`, or on an engine without transactional DDL such as MySQL or Athena. `down()` refuses to run while such a row exists.
...
`repair()` brings the tracking table back in line: it deletes rows left by failed attempts and rewrites stored checksums after an intentional edit, including null checksums on rows written before checksums existed. Repair fixes only the bookkeeping, so before you run it you must clean up by hand any schema changes that a failed attempt left behind. Repair leaves the stored checksum of a changed repeatable alone, so it stays scheduled and the next `up()` re-runs it.
...
- **Drops are opt-in.** Generation leaves extra tables and columns alone unless you pass `allow_drops=True`, which generates the drops. A migration containing drops has no down step, because the dropped data cannot come back. The engine refuses to drop a table that another table's foreign key still names, so a table is dropped before the tables it points at. Tables that point at each other in a cycle lose those keys first. On SQLite, which cannot drop a key, the drops run with foreign key enforcement off, outside a transaction. If you call `autogenerate()` directly, it still raises on undeclared objects. The migrator passes `ignore_undeclared=True`, because hand-written migrations may also touch the database and leave tables that no model declares.
...
Constraint differences generate migrations. A constraint the model declares and the database lacks generates `ADD CONSTRAINT`, with the matching drop as the down step. A changed foreign key generates a drop plus an add under `allow_drops=True`; the down step restores the introspected key when its target is known, and the migration is irreversible otherwise. Every engine that keeps foreign keys reports the target table, the target columns, and the actions: Postgres through `pg_constraint`, MySQL and MariaDB through `referential_constraints`, SQL Server through `sys.foreign_keys`, DuckDB through `duckdb_constraints()`, and SQLite through `PRAGMA foreign_key_list`. A change of target or action therefore diffs. MySQL and MariaDB run `NO ACTION` as `RESTRICT`, so the diff reads the two as the same action there. Constraints in the database that no model declares follow the same rules as extra indexes: they block generation unless `ignore_undeclared` is set, and they drop only under `allow_drops=True`.
...
`AsyncMigrator` reads the same flag and runs the migration outside a transaction. It calls the adapter's `autocommit_scope()` for the run. `DbApiAsyncAdapter` and `AiosqliteAdapter` turn the driver's own transaction control off there and turn it back on after, the same way the blocking migrator does, so a SQLite rebuild's closing `PRAGMA foreign_keys = ON` takes effect. `AsyncpgAdapter` already executes every statement bare, and the base class commits after the run.
...
`migrate --target` runs a shorter set, which has its own key. A rehearsal applies every one of those shorter sets on its way up and takes them all back on the way down, so it records a row for each one that removes data. The rows cover every start point too, because a targeted run leaves a history the next targeted run starts from. A run without a target that follows them applies the rest of the migrations and then the pending repeatables, and the rehearsal records a row for that run from each start point too. One rehearsal therefore covers the whole run, every target within it, and a sequence of targeted runs that walks through it, whether the last step names a target or not.
