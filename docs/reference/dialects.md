...
MySQL and SQL Server restate a whole column to change it, and give a text column the table's or the database's collation unless the statement names one. Their reads take `COLLATION_NAME`, and a generated type or nullability change on a text column restates the collation the catalog reports. MySQL also restates the `ON UPDATE` clause it reports in `EXTRA`. Presto and Trino can neither alter a column nor rebuild a table, so migration generation raises `DialectError` for a column change there. The default dialect cannot alter a column, so migration generation rebuilds the table instead: it creates the new table, copies the rows over, and replaces the old table. SQLite also refuses some new columns in `ADD COLUMN`: a `UNIQUE` column, a `NOT NULL` column with no default, a column whose default is not a constant, such as `CURRENT_TIMESTAMP`, and a column with both `references` and a default. Such a column comes in through a rebuild as well. A rebuild does not reverse, so a migration that contains one has no down step. Dropping the old table needs foreign key enforcement off while another table points at it, and SQLite ignores `PRAGMA foreign_keys` inside a transaction, so such a migration is generated with `transactional=False`. The migrator turns the driver's own transaction control off for that run and puts it back at the end. A failure partway through the run leaves the statements that already ran, the copy table, and enforcement off on that connection. The rebuild keeps the columns and indexes the models do not declare, unless you pass `allow_drops=True`. A model has no way to declare a collation, a `CHECK` without a `CONSTRAINT` name, a `UNIQUE` constraint over more than one column, or a trigger, so the rebuild reads each one off the old table and writes it back whatever `allow_drops` says. A carried check or `UNIQUE` constraint that names a column the rebuild drops is dropped with that column. SQLite checks every view and trigger when it renames a table, and one that names the rebuilt table fails that check while the old table is gone. When the schema has a view or a trigger, the rebuild therefore renames its copy into place between `PRAGMA legacy_alter_table = ON` and `OFF`, and a failure of that rename leaves the setting on for the connection. Introspection cannot read an index on an expression, so a rebuild loses that index and you have to create it again by hand.
...
A snapshot keys every table, column, index, and constraint by its name in lower case, so a model's spelling matches the catalog's on every engine. Each object also keeps the name as the catalog spells it, and a generated statement that drops or restores an object the models do not declare writes that spelling. Postgres takes a quoted name as written, and MySQL on Linux does the same for a table name, so a drop of `"Legacy"` written as `"legacy"` would name a table that is not there.

Postgres has a dedicated read. It takes `udt_name`, varchar lengths, and numeric precision from `information_schema.columns`, every index from `pg_index`, foreign key names, columns, targets, and referential actions from `pg_constraint`, `CHECK` expressions from `pg_constraint` as well, keyed on the table each check belongs to, and enum values from `pg_enum` in sort order. Expression indexes are skipped, as on SQLite.

A check expression is compared after the engine's own rewriting comes off: identifier quoting, the spacing around operators, and parentheses around a single word. A function call keeps its parentheses, so `LENGTH(name) > 5` stays a call. MySQL and MariaDB report `` `price` > 0 `` for `price > 0`, and MSSQL reports `([price]>(0))`, and all three compare equal to the expression the model declares. String literals keep their spelling. Engines can rewrite an expression in ways this comparison does not undo, so a difference that remains appears as a note and never as a drop.
...
A table the read finds in a declared schema keeps that schema in the snapshot, as `IntrospectedTable.schema`. A table in the connection's own schema keeps `None`. Every statement a generated migration writes for a table in a declared schema names the schema in front of the table: the drops of a column, an index, a key, a check, or a whole undeclared table, and a table or column rename. The new name in a table rename stays bare on every engine but MySQL, because the renamed table keeps its schema there, while MySQL moves a table named without a schema to the connection's database. Postgres, DuckDB, and SQLite name no table in `DROP INDEX`, so the index takes the table's schema in front of its own name, as in `DROP INDEX "app"."ix_note"`. Without it, Postgres looks for the index in the schemas on the `search_path`.

A foreign key reads its target table by the bare name, and Postgres, MySQL, MariaDB, and SQL Server add the target's schema as `IntrospectedForeignKey.target_schema` when it is not the schema the connection is on. A declared target such as `app.parents.id` matches a key whose target is `parents` in `app`, and also one whose target the read reports with no schema, because a schema the read leaves out is the connection's own. A bare declared target such as `parents.id` means the connection's schema, so it differs from a key into `app`. A key that a generated migration drops is restored on the way down with its target's schema in front. DuckDB refuses a foreign key across schemas, so its read reports none.
