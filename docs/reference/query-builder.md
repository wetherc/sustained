...
**Relation form.** `joinRelated(relation_name, alias=None)`, and the same prefixed variant for every other join type. The join condition comes from `relationMappings`. A `ManyToManyRelation` joins the through table first, and the join type you name applies to the second hop. The first hop is a `LEFT JOIN` (or `LEFT OUTER JOIN`) for a left or full join, so a row with no link row stays, and an `INNER JOIN` for every other type. A second join through the same link table renders it as `<alias>_<link table>`, and raises `ValueError` when you pass no `alias`. An unknown relation name raises `ValueError`, and so does a mapping that is missing `modelClass`, `join`, `from`, or `to`.
