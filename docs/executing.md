...
An exhausted pool raises `PoolTimeout` after the configured timeout rather than blocking forever. A waiting caller also wakes when the pool drops a broken connection, and it opens a new connection in the freed slot. `pool.close()` closes the idle connections, and a caller still waiting gets `RuntimeError`.
