...
| --- | --- | --- | --- | --- |
| PostgreSQL | 12 and later; suite runs 14 | Container (postgres:14-alpine) | queries, writes, migrations | Rehearsal, advisory locks, and transactional DDL all run here. Enum ADD VALUE inside a transaction sets the floor. |
| MySQL | 8.0.19 and later; suite runs 8.4 | Container (mysql:8.4) | queries, writes, migrations | No transactional DDL, so rehearse needs scratch=True. ALTER TABLE DROP CONSTRAINT sets the floor. |
| MariaDB | 10.6 and later; suite runs 11.4 | Container (mariadb:11.4) | queries, writes, migrations | Same dialect as MySQL. SKIP LOCKED sets the floor. JSON columns read back through their json_valid check. |
| SQL Server | 2012 and later; suite runs 2022 | Container (mcr.microsoft.com/mssql/server:2022-latest) | queries, writes, migrations | Off the rehearsal allowlist, so rehearse refuses without scratch=True. OFFSET with FETCH sets the floor. pyodbc needs the ODBC driver installed. |
...
| SQLite | 3.35 and later | In process | queries, writes, migrations | Standard library. The table rebuild path lives here. RETURNING and DROP COLUMN set the floor; sqlite3.sqlite_version says what your Python links. |
| DuckDB | 1.0 and later | In process | queries, writes, migrations | In-process, so no container and no advisory lock. Releases before 1.0 are not claimed. |
